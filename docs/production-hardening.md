...
Phase 4 implements Cognito JWT verification, local user provisioning, and per-document ownership enforcement in application code. Phase 6B provisioned the temporary VPC/data plane (private RDS, S3, SQS/DLQ), Phase 6C provisioned the non-root deployment identity, Cognito, ECR, secrets, and ECS roles, and Phase 6D deployed the database, API, and worker. Phase 6E now serves the API through trusted HTTPS at `https://api.joshveer.ca` and the frontend through Vercel at `https://know-your-lease-tawny.vercel.app`; Cognito and production CORS use that exact origin with no wildcard. Live checks confirmed exact-origin CORS, rejection of an unrelated origin, unauthenticated `401`, a complete browser code+PKCE login, and authenticated HTTPS upload/rendering of a generated synthetic PDF. The ALB has no HTTP listener, and its previously unused public TCP/80 security-group rule was manually removed via the AWS Console on 2026-09-08; the security group now allows only TCP/443, verified by a subsequent connection failure on port 80 alongside continued healthy HTTPS traffic. Automated DLQ replay, visibility heartbeats, cross-process rate coordination, OCR, malware scanning, restore automation, multi-region infrastructure, and re-ingestion remain later work. Prompt controls reduce injection risk but cannot prove semantic faithfulness, and abstention has no calibrated relevance threshold.
