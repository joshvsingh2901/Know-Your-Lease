...
Ingestion resolves the document row's storage key, calls `DocumentStorage.read`, and passes the bytes to the existing PyMuPDF extraction path. Chunking, Voyage embeddings, status transitions, retrieval, Gemini generation, citation verification, and answer caching are unchanged. In SQS mode the standalone worker performs this work with the same storage abstraction; local inline mode retains the background task for development.
