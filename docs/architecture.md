...
The current system implements Cognito authentication and per-document ownership in application code, and its Phase 6B S3/SQS/RDS/network resources exist, but it does not provision a live Cognito pool, ECS workloads, public ALB, ECR repository, or application IAM/secrets. Pgvector is requested by the base migration but is not verified on RDS until the later in-VPC migration task runs. Automated DLQ replay, visibility heartbeats, OCR, cross-process rate limiting, chat history, query rewriting, reranking, hybrid/full-text retrieval, coordinate-level PDF highlights, and a calibrated relevance threshold remain absent. There is also no re-ingestion endpoint or other producer that advances a document past ingestion version 1.
