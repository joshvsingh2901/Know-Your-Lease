...
Stage 6B keeps the RAG pipeline unchanged while hardening its boundary. Every document API route first resolves through a central access dependency, then retrieval, orchestration, and cache queries retain their own `document_id` predicates. Today the dependency intentionally represents one local user; authenticated ownership must later be added there. Provider keys and the database URL are secret-typed configuration, logs record safe operational fields rather than prompts/provider bodies, and production startup rejects wildcard/invalid CORS origins, loopback or non-HTTPS frontend origins, default database settings, missing provider keys, enabled debug routes, and storage under frontend public assets.
