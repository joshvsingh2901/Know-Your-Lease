...
The final retrieved chunks receive deterministic identifiers (`SOURCE_1`, `SOURCE_2`, and so on). Gemini receives only these excerpts and the question as JSON-encoded untrusted data, separated from the trusted system instruction. Its system instruction prohibits outside legal knowledge, invented lease terms, legal advice, and following instructions embedded in either the document or the user's question. It also asks Gemini to synthesize qualifying or apparently conflicting provisions instead of presenting an unqualified yes/no or resolving enforceability itself.

The backend validates every returned source ID against the supplied evidence map in the generation adapter and again in question orchestration. Unknown IDs reject the response, duplicate valid IDs are removed in order, and page numbers are never accepted from Gemini. Citations are built from database-owned chunk IDs, page numbers, nullable section titles, exact chunk text, and retrieval scores. Model quotes are used only after normalized containment verification; a response without valid citations is forced to the fixed abstention answer and is not cached.
...
Stage 6B keeps the RAG pipeline unchanged while hardening its boundary. Every document API route first resolves through a central access dependency, then retrieval, orchestration, and cache queries retain their own `document_id` predicates. Phase 4 added the authenticated-owner predicate to that same dependency (`Document.owner_id == current_user.id`) rather than adding it per route; see `docs/authentication.md`. Provider keys and the database URL are secret-typed configuration, logs record safe operational fields rather than prompts/provider bodies, and production startup rejects wildcard/invalid CORS origins, loopback or non-HTTPS frontend origins, default database settings, missing provider keys, enabled debug routes, storage under frontend public assets, and a missing/disabled `AUTH_MODE`.
...
Inline development schedules ingestion with a FastAPI background task. SQS mode instead commits `queued`, publishes the document UUID plus ingestion version, and lets the standalone worker invoke the same ingestion service. Chunks are inserted only after extraction, chunking, and all embeddings succeed. One final transaction validates the current version/attempt, replaces chunks, invalidates cached answers, records the completed version, and marks `ready`. A rollback preserves the previously committed index and cache while the non-ready status blocks them from serving.

Atomic conditional claims plus short row-locked completion checks make duplicate, stale, reclaimed, and concurrently delivered work idempotent. Completed, stale, and recorded-terminal-failure receipts are deleted, since those outcomes are already durably persisted on the document row; all retryable, malformed, missing, future, or busy work remains for AWS visibility/redrive behavior. Brief Voyage retries remain bounded before SQS becomes the outer retry layer. The application never manually republishes normal retries or sends directly to a DLQ.

## Current limitations

OCR, a provisioned/live Cognito user pool, provisioned S3/SQS/DLQ infrastructure, automated DLQ replay, visibility heartbeats, cross-process provider coordination, chat history, reranking, hybrid retrieval, calibrated relevance thresholds, and coordinate-level PDF highlights are not implemented. Debug chunk/retrieval endpoints expose lease excerpts, require both an authenticated owner and `DEBUG_ENDPOINTS_ENABLED`, and must remain disabled in production.
