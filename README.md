...
The API, worker, and migration task use the same Docker image with workload-specific validation and least-secret configuration. Phase 6C provisioned a non-root deployment identity, Cognito, ECR, and ECS roles; Phase 6D deployed the database, API, and worker; Phase 6E added ACM/HTTPS, `api.joshveer.ca`, exact-origin Cognito/CORS configuration, and the Vercel frontend at `https://know-your-lease-tawny.vercel.app`. See the [deployment overview](docs/deployment.md), [AWS deployment preparation](docs/aws-deployment.md), [AWS identity runbook](docs/aws-identity.md), and [live resource inventory](docs/aws-resource-inventory.md).
...
- Cognito authentication and per-document ownership are deployed; production callback/logout URLs, exact-origin CORS, browser code+PKCE login, and authenticated HTTPS retrieval of a generated synthetic PDF are verified
...
Further detail: [architecture](docs/architecture.md), [AWS deployment preparation](docs/aws-deployment.md), [AWS identity runbook](docs/aws-identity.md), [authentication](docs/authentication.md), [document storage](docs/document-storage.md), [RAG decisions](docs/rag-decisions.md), [retrieval evaluation](docs/retrieval-evaluation.md), and [production hardening](docs/production-hardening.md).
